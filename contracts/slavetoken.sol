...
contract SlaveToken is SlaveAction, ERC721 {
  ...
  function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes calldata _data) external onlyOwnerOf(_tokenId){
    require(slaveToOwner[_tokenId] == msg.sender || slaveApprovals[_tokenId] == msg.sender, "Transfer not accepted by both parties.");
    _transfer(_from, _to, _tokenId);
  }

  function safeTransferFrom(address _from, address _to, uint256 _tokenId) external onlyOwnerOf(_tokenId) {
    require(slaveToOwner[_tokenId] == msg.sender || slaveApprovals[_tokenId] == msg.sender, "Transfer not accepted by both parties.");
    _transfer(_from, _to, _tokenId);
  }
  ...
  function setApprovalForAll(address _operator, bool _approved) external {
    require(false, _generateAnnoyingErrorMessage());
  }
